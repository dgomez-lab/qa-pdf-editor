...
Tras cada regresión (PR o `profile: regression`), el job **Publish regression report** fusiona hasta **18** fragmentos NDJSON (14 funcionales + 4 visuales) y los PNG de `failure-artifacts-*`, y publica un dashboard HTML (cuadrícula, passed/failed/skipped, pasos Gherkin con mensaje de error, capturas en fallos). El merge deduce **PASSED/FAILED** desde los pasos Gherkin cuando playwright-bdd no envía `testCaseResult` en `testCaseFinished`.
...
Cada shard sube `cucumber-messages-*` y `blob-report-*` siempre. Si falla, sube además `failure-artifacts-*` con capturas y trazas:
...
| Sin screenshots en fallos del dashboard | Artefactos `failure-artifacts-*` no indexados o sin `manifest.ndjson` | Confirmar que el shard falló y subió `cucumber-report/failure-screenshots/`; el merge actual indexa PNG + `manifest.ndjson` |
