...
- `ci-fast` fija `PLAYWRIGHT_CI_RETRIES=2`; la regresión en PR fija `0` reintentos para que los shards muestren fallos reales sin ocultarlos; `workflow_dispatch` de regresión usa `1`.
...
| Sin screenshots en fallos del dashboard | Artefactos `failure-artifacts-*` sin PNG/`manifest.ndjson` bajo `cucumber-report/failure-screenshots/` | Revisar upload del shard; el merge indexa capturas desde esos artefactos |
