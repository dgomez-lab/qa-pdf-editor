...
Cada **PR** a `main`/`master` ejecuta **fast** (~1–2 min) y luego regresión paralela (**14** shards funcionales + **4** visuales, tope 40 min por shard). **Push** a `main`/`master` solo ejecuta **fast**. Manual: **Run workflow** con perfil **`regression`** (por defecto). Ver [docs/GITHUB_REGRESSION.md](docs/GITHUB_REGRESSION.md).
