...
Cada **PR** a `main` ejecuta **fast** (~1–2 min) y luego regresión paralela (**14** shards funcionales + **4** visuales, tope 40 min/shard). **Push** a `main` solo ejecuta **fast**. Manual: **Run workflow** con perfil **`regression`** (por defecto). Ver [docs/GITHUB_REGRESSION.md](docs/GITHUB_REGRESSION.md).
...
| `npm run test:unit` | Tests unitarios de helpers (`tests/helpers/**/*.spec.ts`, `playwright/**/*.spec.ts`) sin generar specs BDD. |
