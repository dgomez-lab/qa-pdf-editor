...
/**
 * `Recurrences.feature` — `@PDFEDITOR_PAYMENT_RECURRENCE_LEGACY_14056_SOFT_DECLINE`:
 * dispara 14056 soft decline → Padrina Failed en CRM.
 */
test.describe('Recurrences — legacy 14056 (soft decline)', { tag: ['@PDFEDITOR_PAYMENT'] }, () => {
  test.beforeEach(() => {
    test.skip(!crmReady(), 'PLAYWRIGHT_CRM_USER/PASSWORD')
    test.skip(!recurrenceApiReady(), 'PLAYWRIGHT_RECURRENCE_API_BASE_URL')
  })

  test('14056 soft decline → Padrina Failed', { tag: ['@PDFEDITOR_PAYMENT_RECURRENCE_LEGACY_14056_SOFT_DECLINE'] }, async ({ page }) => {
    ...
    await payLegacyRecurrence(subscriptionId, 'soft')
