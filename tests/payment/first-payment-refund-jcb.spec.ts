...
test.describe('First payment refund — JCB', { tag: ['@PDFEDITOR_PAYMENT', '@PDFEDITOR_PAYMENT_FIRST_REFUND_JCB'] }, () => {
  test.beforeEach(() => {
    test.skip(!paymentSmokeEnabled(), 'PLAYWRIGHT_PAYMENT_SMOKE=1')
    test.skip(!crmReady(), 'PLAYWRIGHT_CRM_USER/PASSWORD')
  })

  test('pago JCB + refund', async ({ page, context }) => {
    test.setTimeout(420_000)
    await runFirstPaymentRefund(page, context, {
      number: process.env.STRIPE_TEST_JCB_NUMBER ?? '3528000700000000',
