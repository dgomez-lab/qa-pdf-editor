...
function resolveStepText(testCase, testStepId, pickleStepText) {
  const testStep = testCase?.testSteps?.find((s) => s.id === testStepId)
  const pickleStepId = testStep?.pickleStepId || testStepId
  return pickleStepText.get(pickleStepId) || testStepId
}
    ...
    if (env.testStepStarted) {
      ...
      if (att) {
        ...
        att.steps.push({
          ...
          text: resolveStepText(testCase, env.testStepStarted.testStepId, pickleStepText),
          status: 'UNKNOWN',
          errorMessage: ''
        })
      }
