...
function applyReadableStepText(attempts, testStepText, pickleStepText) {
  for (const att of attempts.values()) {
    for (const step of att.steps) {
      step.text = testStepText.get(step.id) || pickleStepText.get(step.id) || step.text || step.id
    }
  }
}
...
async function parseCucumberFailures(filePath) {
  ...
  applyReadableStepText(attempts, buildTestStepTextMap(testCases, pickleStepText), pickleStepText)

  const failures = []
  for (const [startedId, att] of attempts) {
    ...
    const failedStep = att.steps.find((s) => s.status === 'FAILED' && !isHookStepId(s.id))
    const gherkinFailed = att.steps.filter((s) => !isHookStepId(s.id) && s.status === 'FAILED')
    failures.push({
      tag,
      scenarioName: pickle?.name || startedId,
      failedStep: failedStep?.text || gherkinFailed[0]?.text || '(unknown step)',
...
function buildMarkdown(playwrightFailures, cucumberFailures) {
  if (playwrightFailures.length === 0 && cucumberFailures.length === 0) {
  ...
  lines.push('Artifacts: `playwright-report-fast` (HTML, traces, `test-results/`, `cucumber-report/messages.ndjson`, failure screenshots).')
  lines.push('')
  return lines.join('\n')
}
...
function isMainModule() {
  return process.argv[1] && currentFile === path.resolve(process.argv[1])
}
